...
module.exports = {
  parseAddress: function (address) {
    ...
    if (
      countrySection === "US" ||
      countrySection === "USA" ||
      countrySection === "United States" ||
      countrySection === "Canada"
    ) {
    ...
    if (stateString.match(/\d{5}$/)) {
      ...
      stateString = stateString
        .substring(0, stateString.length - result.zipCode.length)
        .trim();
    }
    // Parse and remove state
    if (stateString.length > 0) {
      // Check if anything is left of last section
    ...
    if (
      ...
      getKeyByValue(allStates, stateString.toUpperCase())
    ) {
      result.stateAbbreviation = stateString.toUpperCase();
      result.stateName = toTitleCase(
        getKeyByValue(allStates, stateString.toUpperCase())
      );
    ...
    var streetString = "";
    var usStreetDirectionalString = Object.keys(usStreetDirectional)
      .map((x) => usStreetDirectional[x])
      .join("|");
    ...
    if (addressParts.length === 1) {
      ...
      var reStreet = new RegExp(
        ".*\\b(?:" +
          Object.keys(usStreetTypes).join("|") +
          ")\\b\\.?" +
          "( +(?:" +
      ...
      if (streetString.match(reAveLetter)) {
        ...
        if (streetString && streetString.length > 0) {
          ...
          if (
            result.hasOwnProperty("addressLine2") &&
            result.addressLine2.length > 0
          ) {
            throw (
        ...
        // Normalize to Ave
        streetParts[streetParts.length - 2] = streetParts[
          streetParts.length - 2
        ].replace(/^(ave.?|avenue)$/i, "Ave");
        ...
        if (streetParts.length > 2) {
          ...
          streetParts[streetParts.length - 1] = streetParts[
        ...
        if (result.streetDirection) {
          result.addressLine1 =
            result.addressLine1 + " " + result.streetDirection;
        }
        ...
        var reLine2 = new RegExp(
          "\\s(" + usLine2String + ")\\.?\\s[a-zA-Z0-9_-]+$",
          "i"
        );
    ...
    if (
      addressString &&
      result.hasOwnProperty("placeName") &&
      result.hasOwnProperty("stateAbbreviation") &&
      result.hasOwnProperty("zipCode")
    ) {
      var idString =
        addressString +
        ", " +
  ...
  randomCity: function () {
    ...
    var randomCityElementId = Math.floor(
      Math.random() * randomStateData.length
    );
